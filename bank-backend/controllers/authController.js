...
const {
  generateUserToken,
  generateAdminToken,
} = require("../utils/tokenregistration");
...
const createAdmin = async (req, res) => {
  ...
  try {
    ...
    res.send(
      "Account created. Check to see whether you are allowed or not. (If you are not allowed once, do not attempt again! Necessary actions will be taken otherwise."
    );
  } catch (err) {
    console.error(err);
...
const loginAdmin = async (req, res) => {
  ...
  if (!admin)
    return res
      .status(400)
      .json("You haven't attempted to access admin priviliges.");
  if (admin.permission !== true) {
    return res.send("You are currently not allowed admin priviliges");
  }
...
const permissionAdmin = async (req, res) => {
  if (req.admin.email !== "admin-1@gmail.com") {
    return res.send("You are not allowed to give permissions");
  }
